...
def _quad_score(quad: np.ndarray, img_shape: tuple) -> float:
    ...
    w_ = (np.linalg.norm(rect[1]-rect[0]) +
          np.linalg.norm(rect[2]-rect[3])) / 2
    h_ = (np.linalg.norm(rect[3]-rect[0]) +
          np.linalg.norm(rect[2]-rect[1])) / 2
    if w_ < 10 or h_ < 10:
        return -1.0
...
def _best_binarise(cell: np.ndarray) -> np.ndarray:
    ...
    for src_cell in (cell, clahe_cell):
        for m in ('a2', 'a4', 'a6', 'otsu', 'otsu_inv', 'fixed'):
            try:
                b = _binarise(src_cell, m)
                s = _bin_score(b)
                if s > best_s:
                    best_s, best_b = s, b
            except Exception:
...
def _consistent(board: list) -> bool:
    for i in range(81):
        ...
        br, bc = (r//3)*3, (c//3)*3
        for j in range(9):
            if j != c and board[r*9+j] == v:
                return False
            if j != r and board[j*9+c] == v:
                return False
        for dr in range(3):
            for dc in range(3):
                ni = (br+dr)*9+(bc+dc)
                if ni != i and board[ni] == v:
                    return False
    return True
...
def _filter_stages(img, gray, corners, warped) -> dict:
    ...
    if corners is not None:
        ...
        for pt in pts:
            cv2.circle(draw, tuple(pt.astype(int)), 8, (0, 0, 255), -1)
    blr = cv2.GaussianBlur(gray, (5, 5), 0)
